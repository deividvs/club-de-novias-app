migrate(
  (app) => {
    ...
    const weddings = app.findRecordsByFilter('weddings', '1=1', '', 10000, 0)
    ...
    for (const wedding of weddings) {
      ...
      const existingTitles = new Set(existingTasks.map((t) => t.getString('title')))

      for (const group of plannerTasks) {
        for (const title of group.titles) {
          if (!existingTitles.has(title)) {
            const record = new Record(tasksCollection)
            record.set('wedding_id', wedding.id)
    ...
    for (const group of plannerTasks) {
      for (const title of group.titles) {
        const records = app.findRecordsByFilter(
          'tasks',
          `title = '${title.replace(/'/g, "''")}' && category = '${group.category}'`,
          '',
          10000,
          0,
        )
